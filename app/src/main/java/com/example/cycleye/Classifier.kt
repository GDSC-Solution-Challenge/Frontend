...
class Classifier(private var context: Context, private val modelName: String) {
    ...
    fun init(label_file_url: String) {
        model = Model.createModel(context, modelName)
        initModelShape()
        labels.addAll(FileUtil.loadLabels(context, label_file_url))
    }
    ...
    companion object {
        const val CLASSIFY_MODEL = "models/classification_model.tflite"
        const val OX_MODEL = "models/baseOX_230329.tflite"
    }
}